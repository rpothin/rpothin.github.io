...
- The best way to secure access to your Power Platform environments is to configure an Azure AD security group on each one of them. If a user is not a member of the Azure AD group configured on an environment, it will not be possible to add them as a user.
  ...
  ![Error message trying to add a user not member of the considered Azure AD group to a Power Platform environment](/content/posts/ppic-env-group-error.png)
...
![Power Platform Tenant Isolation enabled without rules](/content/posts/ppic-tenant-isolation-enabled.png)

![Error on a connection in an existing cloud flow after enabling Power Platform Tenant Isolation](/content/posts/ppic-tenant-isolation-flow-error.png)
...
![Where to find a tenant domain or ID for the configuration of Tenant Isolation rules](/content/posts/ppic-tenant-domain-id.png)
...
![Example of an outbound rule configured in the Power Platform Tenant Isolation page](/content/posts/ppic-tenant-isolation-outbound-rule.png)

![Error trying to configure a connection to a tenant without inbound rules in the Tenant Isolation configuration](/content/posts/ppic-tenant-isolation-inbound-error.png)
...
![Illustration of the activation of the “IP address based cookie binding” feature](/content/posts/ppic-ip-cookie-binding.png)
...
![Error displayed in the browser if the IP of a user changes](/content/posts/ppic-ip-change-error.png)

![Similar error got by a malicious actor executing requests with an exfiltrated cookie](/content/posts/ppic-exfiltrated-cookie-error.png)
...
![Simple configuration of IP address based firewall rule](/content/posts/ppic-ip-firewall-rule.png)
...
![IPFirewallAccessDenied audit log example to illustrate the feature in audit mode](/content/posts/ppic-ip-firewall-audit-log.png)
...
![Activation of Customer Lockbox in Power Platform Admin Center](/content/posts/ppic-customer-lockbox.png)
