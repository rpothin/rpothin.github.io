...
description: "Canonical content quality standards for this developer blog. Defines frontmatter schemas (new posts and archive), code-block classification and language-annotation rules, GitHub alert formatting, link validation procedures, slug naming, tag conventions, media guidelines, and the confidence threshold pattern. Use when creating, polishing, or migrating Markdown content under content/posts/ or content/archive/."
---

# Content Standards

Canonical rules for all Markdown content on this site. Both the `content-ghostwriter` and `migration-ghostwriter` agents apply these standards. This skill is also available via `/content-standards` for manual content work.

## Procedure

Apply these checks in order when creating or polishing a Markdown file:

1. **Validate frontmatter** — enforce the correct schema (new-post or archive). See [Frontmatter](#frontmatter).
2. **Remove duplicate first heading** — if the body starts with a `#` that matches the title. See [First Heading](#first-heading).
3. **Classify and annotate code blocks** — distinguish code from prose-in-fences; add language tags or convert to alerts/lists. See [Code Blocks](#code-blocks).
4. **Format GitHub alerts** — convert callout-style content to `[!NOTE]`, `[!TIP]`, etc. where it genuinely fits. See [GitHub Alerts](#github-alerts).
5. **Validate links** — check every external URL; update redirects, remove dead links. See [Link Validation](#link-validation).
6. **Verify slug and media conventions** — confirm naming and image/video references follow the rules. See [Slug Naming](#slug-naming) and [Media](#media).
