...
date: 2023-06-15
tags: [power-platform, security, entra-id, conditional-access, zero-trust, series-power-platforms-protection]
...
![Overview of Azure AD Conditional Access](/content/posts/aad-ca-overview.png)

> [!NOTE]
> **Example:** If a user in group A tries to access Dataverse from outside of the United States, block the request.

![Illustration of the Signal > Decision > Enforcement flow in Azure AD Conditional Access](/content/posts/aad-ca-signal-decision-enforcement.png)
...
![Power Platform apps available in Azure AD Conditional Access policies](/content/posts/aad-ca-power-platform-apps.png)
...
- Requiring multifactor authentication (MFA) for Power Platform Administrators accessing Power Platform services
  ...
  ![Requiring MFA for Power Platform Administrators — Included apps](/content/posts/aad-ca-mfa-admin-included-apps.png)

  ![Requiring MFA for Power Platform Administrators — Selected grant control](/content/posts/aad-ca-mfa-admin-grant.png)
...
- Blocking a user trying to access Dataverse data from a desktop app (like Excel)
  ...
  ![Block access to Dataverse from mobile / desktop — Included apps](/content/posts/aad-ca-block-mobile-included-apps.png)

  ![Block access to Dataverse from mobile / desktop — Selected client apps as condition](/content/posts/aad-ca-block-mobile-client-apps.png)

  ![Block access to Dataverse from mobile / desktop — Block access](/content/posts/aad-ca-block-mobile-grant.png)

  ![Block access to Dataverse from mobile / desktop — Test result from Excel](/content/posts/aad-ca-block-mobile-test.png)
...
- Redirecting to Defender for Cloud Apps users accessing Dataverse from outside United States to apply more security rules (like blocking files download)
  ...
  ![Redirect access to Dataverse from outside United States to Defender for Cloud Apps — Included apps](/content/posts/aad-ca-redirect-usa-included-apps.png)

  ![Redirect access to Dataverse from outside United States to Defender for Cloud Apps — Location condition](/content/posts/aad-ca-redirect-usa-location.png)

  ![Redirect access to Dataverse from outside United States to Defender for Cloud Apps — Custom policy for Conditional Access App](/content/posts/aad-ca-redirect-usa-mcas-policy.png)

  ![Redirect access to Dataverse from outside United States to Defender for Cloud Apps — Test result from Canada](/content/posts/aad-ca-redirect-usa-test.png)

> [!NOTE]
> This scenario is explored further in the [Defender for Cloud Apps article](/posts/02-defender-for-cloud-apps).
...
![Azure AD Conditional Access Authentication Context to link to a canvas app](/content/posts/aad-ca-auth-context.png)

![Canvas apps considered in the examples — Only using the User() object](/content/posts/aad-ca-canvas-apps-examples.png)
...
- Blocking the access to a canvas app for a user if not on a specific device configuration (browser not allowed)
  ...
  ![Block access to canvas app if user not on an iOS device — Included authentication context](/content/posts/aad-ca-block-ios-auth-context.png)

  ![Block access to canvas app if user not on an iOS device — Condition on device platform](/content/posts/aad-ca-block-ios-platform.png)

  ![Block access to canvas app if user not on an iOS device — Block access](/content/posts/aad-ca-block-ios-grant.png)

  ![Block access to canvas app if user not on an iOS device — Test result from a browser on a Windows device](/content/posts/aad-ca-block-ios-test.png)

- Requiring multifactor authentication for a user with accessing a canvas app with a medium user and sign-in risk

  ![Require MFA for a user with medium user or sign-in risk — Included authentication context](/content/posts/aad-ca-mfa-risk-auth-context.png)

  ![Require MFA for a user with medium user or sign-in risk — Medium user risk condition](/content/posts/aad-ca-mfa-risk-condition.png)

  ![Require MFA for a user with medium user or sign-in risk — Require MFA grant control](/content/posts/aad-ca-mfa-risk-grant.png)
...
1. Get the IP ranges of the AppService Azure Service Tags of the Canada East Azure region using the `az network list-service-tags` command from Azure CLI
   ...
   ![Get AppService.CanadaEast Azure Service Tag IP Ranges](/content/posts/aad-ca-canada-east-ip-ranges.png)
...
2. Create a Named Location with IP ranges in Azure AD Conditional Access with the result from step 1 using the `New-MgIdentityConditionalAccessNamedLocation` command of the Microsoft Graph PowerShell module
   ...
   ![Creation of Azure AD Conditional Access Named Location for App Service Canada East IP ranges](/content/posts/aad-ca-canada-east-named-location.png)
...
![Block access to Dataverse for workload identities not coming from Azure Functions Canada East — Included apps](/content/posts/aad-ca-workload-identity-included-apps.png)

![Block access to Dataverse for workload identities not coming from Azure Functions Canada East — Excluded locations](/content/posts/aad-ca-workload-identity-excluded-locations.png)

![Block access to Dataverse for workload identities not coming from Azure Functions Canada East — Block access](/content/posts/aad-ca-workload-identity-grant.png)

![Block access to Dataverse for workload identities not coming from Azure Functions Canada East — Test result from my laptop](/content/posts/aad-ca-workload-identity-test.png)
...
2. [Power Platform's protection — Defender for Cloud Apps](/posts/02-defender-for-cloud-apps)
3. [Power Platform's protection — Platform internal capabilities](/posts/03-platform-internal-capabilities)
4. [Power Platform's protection — Microsoft Purview Compliance](/posts/04-microsoft-purview-compliance)
5. [Power Platform's protection — Virtual Network integration](/posts/05-virtual-network-integration)
6. [Power Platform's protection — Managed Identity for Dataverse plug-ins](/posts/06-managed-identity-for-dataverse-plug-ins)
7. [Power Platform's Protection — Building secure and responsible Generative AI solutions](/posts/07-building-secure-and-responsible-generative-ai-solutions)
8. [Power Platform's Protection — Microsoft Purview the data guardian](/posts/08-microsoft-purview-the-data-guardian)
9. [Power Platform's Protection — Microsoft Sentinel, the watchtower](/posts/09-microsoft-sentinel-the-watchtower)
